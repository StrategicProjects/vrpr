#!/usr/bin/env Rscript
# Parity benchmark of vrpr vs PyVRP on a CVRPLIB instance.
#
# Downloads an X-set instance (Uchoa et al.) if absent, solves it with vrpr and --
# if a Python with PyVRP is available -- also with PyVRP, and compares the costs to
# the known optimum (from the .sol file).
#
# Usage:  Rscript tools/benchmark/parity.R [instance] [seconds] [seed]
#    e.g. Rscript tools/benchmark/parity.R X-n101-k25 10 1
#
# For the PyVRP side, set VRPR_PYVRP_PYTHON to the python that has pyvrp installed
# (otherwise it tries "python3"); if unavailable, it compares only vrpr vs the optimum.

suppressMessages(library(vrpr))

# Minimal JSON parser (avoids a {jsonlite} dependency).
...
# Number of vehicles: a generous upper bound (CVRP minimises distance only).
n_veh <- 30L

cli::cli_h1("Parity vrpr vs PyVRP — {instance} ({secs}s, seed {seed})")
cli::cli_alert_info("Known optimum (BKS): {bks}")
...
cli::cli_alert_success(
  "vrpr:  cost {round(cost(res))}  gap {gap(cost(res))}  \\
   ({res$iterations} iterations in {round(vrpr_secs, 1)}s)"
)
...
if (has_py && file.exists(script)) {
  ...
  cli::cli_alert_success(
    "PyVRP: cost {pj$cost}  gap {gap(pj$cost)}  ({pj$iterations} iterations)"
  )
  cli::cli_alert_info(
    "PyVRP/vrpr iteration ratio: {round(pj$iterations / res$iterations, 1)}x"
  )
} else {
  cli::cli_alert_warning(
    "PyVRP unavailable (set VRPR_PYVRP_PYTHON). Comparing only vrpr vs the optimum."
  )
}
