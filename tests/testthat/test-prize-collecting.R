# Prize-collecting: clientes opcionais com prêmio.
pc_model <- function() {
  clientes <- tibble::tibble(
    ...
    prize    = c(0, 0, 0, 5, 500, 5) # só o do meio (500) compensa o desvio
  )
  vrp_model() |>
    add_depot(0, 0) |>
...
test_that("clientes opcionais de baixo prêmio são pulados; os de alto, visitados", {
  res <- vrp_solve(pc_model(), stop = max_iterations(500), seed = 1, display = FALSE)
  rt <- routes(res)
  expect_true(all(1:3 %in% rt$client)) # obrigatórios
  expect_true(5 %in% rt$client)        # opcional de prêmio 500
  expect_false(any(c(4, 6) %in% rt$client)) # opcionais de prêmio 5
})

test_that("uncollected_prizes e unvisited_clients são coerentes", {
  res <- vrp_solve(pc_model(), stop = max_iterations(500), seed = 1, display = FALSE)
  expect_equal(res$solution$summary$uncollected_prizes, 10) # 5 + 5
  expect_setequal(unvisited_clients(res), c(4L, 6L))
})

test_that("o objetivo inclui os prêmios não coletados", {
...
test_that("grupo obrigatório: exatamente um membro é visitado (o mais próximo)", {
  ...
  do_grupo <- intersect(routes(res)$client, c(2, 3, 4))
  expect_length(do_grupo, 1L)
  expect_equal(do_grupo, 2L) # o mais próximo do depósito
  expect_true(res$solution$summary$is_complete)
})

test_that("grupo opcional de baixo prêmio: nenhum membro visitado", {
  m <- vrp_model() |>
    add_depot(0, 0) |>
...
test_that("validações de grupo", {
  ...
  dois <- base |>
    ...
    add_client_group(clients = 1)
  expect_error(vrp_problem_data(dois), "dois grupos")
})
