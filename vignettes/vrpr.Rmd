...
A *shipment* pairs a pickup point with a delivery point: the same vehicle must
visit the pickup first and then the delivery, in the same trip. Add shipments
with `add_shipments()`; they can be mixed freely with regular clients, time
windows and every other feature.
...
The `activity` column distinguishes pickups from deliveries, and `shipment`
identifies the pair. Optional shipments (`required = FALSE`, with a `prize`)
may be left out; `unplanned()` lists them.
